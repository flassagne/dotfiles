...
#   ---------------------------------------
#   8. SPECIAL WORK CONFIG
#   ---------------------------------------
   cd_proj() { builtin cd ~/work/source/$@; }
   alias cd_source="cd ~/work/source"
   alias cd_build="cd ~/work/build"
   alias cd_install="cd ~/work/install"
...
#   9. TOOLS DEVELOPMENT
#   ---------------------------------------
   recurs-sed(){ sed -i '$2' $(grep -r $1 | awk -F':' '{print $1}' | xargs ) ; }

   recurs-rename(){ find . -name *$1* -exec rename s/$1/$2/g dirname {} \; ; }
